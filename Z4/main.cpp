...
public:
  ...
  bool operator==(const Datum &d) {
    if (dan == d.dajDan() && mjesec == d.dajMjesec() && godina == d.dajGodinu())
      return true;
    return false;
  }
};
...
public:
  Vrijeme(int sati = 0, int minute = 0) { Postavi(sati, minute); }
...
public:
  Pregled(const std::string &ime, const Datum &dat, const Vrijeme &v)
      : ime(ime), datum(dat), vrijeme(v) {}
  Pregled(const std::string &ime, int d, int m, int g, int h, int min)
      : ime(ime) {
    datum = {d, m, g};
    vrijeme = {h, min};
  }
...
public:
  ...
  void RegistrirajPregled(const std::string &ime, const Datum &dat,
                          const Vrijeme &v) {
    pregledi.push_back(std::make_shared<Pregled>(ime, dat, v));
  }
  void RegistrirajPregled(const std::string &ime, int d, int m, int g, int h,
                          int min) {
    pregledi.push_back(std::make_shared<Pregled>(ime, d, m, g, h, min));
  }
  void RegistrirajPregled(std::shared_ptr<Pregled> p) { pregledi.push_back(p); }
  int DajBrojPregleda() const { return pregledi.size(); }
  int DajBrojPregledaNaDatum(Datum dat) const {
    return std::count_if(std::begin(pregledi), std::end(pregledi),
                         [&dat](std::shared_ptr<Pregled> p) {
                           if (dat == p->DajDatumPregleda())
                             return true;
                           return false;
                         });
  }
  ...
  Pregled &DajNajranijiPregled() {
    ...
    return **std::min_element(std::begin(pregledi), std::end(pregledi),
                              [](const std::shared_ptr<Pregled> &p1,
                                 const std::shared_ptr<Pregled> &p2) {
  ...
  void IspisiSvePreglede() const{
    std::vector<std::shared_ptr<Pregled>> sortirani_pregledi = pregledi;
    std::sort(std::begin(sortirani_pregledi), std::end(sortirani_pregledi),
              [](const std::shared_ptr<Pregled> &p1,
                 const std::shared_ptr<Pregled> &p2) {
                return Pregled::DolaziPrije(*p1, *p2);
              });
    for (int i = 0; i < sortirani_pregledi .size(); i++) {
      sortirani_pregledi [i]->Ispisi();
    }
  }
};
...
Pregledi::Pregledi(const Pregledi &p): pregledi(p.pregledi.size()){
 ...
 Pregledi::Pregledi(std::initializer_list<Pregled> p){
     for(const auto &pom : p){
         pregledi.push_back(std::make_shared<Pregled>(pom));
     }
 }
...
void Pregledi::ObrisiNajranijiPregled() {
  ...
  // pregledi.erase(pom);
  for (int i = 0; i < pregledi.size(); i++) {
    if (&pom == pregledi[i].get()) {
      pregledi.erase(std::begin(pregledi) + i);
      break;
    }
  }
}
...
void Pregled::PomjeriDanUnaprijed() {
  int dan = datum.dajDan(), mjesec = datum.dajMjesec(),
      godina = datum.dajGodinu();
